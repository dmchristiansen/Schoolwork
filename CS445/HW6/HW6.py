...
class Robot:
    ...
    def read_sensor(self):
        ...
        sensor = (N, S, E, W, H)
        """
        sensor = np.zeros(5, dtype=np.int)
        sensor[0] = self.grid[self.y 1 1, self.x]
        sensor[1] = self.grid[self.y + 1, self.x]
        sensor[2] = self.grid[self.y, self.x + 1]
        sensor[3] = self.grid[self.y, self.x - 1]
        sensor[4] = self.grid[self.y, self.x]

        # Updates and returns sensor values interpreted as a base-3 value.
        self.previous_state = self.state
        self.state = np.dot(sensor, [81, 27, 9, 3, 1])
        return sensor

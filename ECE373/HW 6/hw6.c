...
/*
	initial sequence:
	__init() sets up char driver stuff
	probe() sets up pci stuff
	alloc/pin memory for descriptor ring using dma_alloc_coherent()
		(calls kmalloc, maps physical address to kernel logical address, returns both)
	for each descriptor, create an associated buffer
		(kmalloc() -> kernel logical -> dma_map_single() -> physical -> cpu_to_le64())
...
static struct rx_ring {
	struct rx_desc * ring_addr;
	size_t size;
	unsigned int count;
...
// set up descriptor ring & buffers for DMA
static int dma_setup(struct pci_dev *pdev)
{
	dring.ring_addr = dma_alloc_coherent(pdev
...
static int pci_probe(struct pci_dev *pdev, const struct pci_device_id *pdid)
{
	...
	if(dma_setup(pdev))
		printk(KERN_INFO "dma setup failed.\n");
